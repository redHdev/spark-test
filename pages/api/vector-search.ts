...
import { functions } from '../../components/spark-gpt/states/gameState';
...
export default async function handler(req: NextRequest) {
  try {
    ...
    const { query, promptConfig } = JSON.parse(Prompt);
    ...
    const backendCompanions = promptConfig?.backendCompanions || null;
    
    const companionId = backendCompanions?.cCompanion || null;

    const cIntros = backendCompanions?.cIntros ? `When the user greets you with "hey", "hi", "hello" etc. respond with this announcement/faq: ${backendCompanions.cIntros}` : '';

    const cCharacters = backendCompanions?.cCharacters ? `This is the character you must potray as: ${backendCompanions.cCharacters}` : '';

    const cExtras = backendCompanions?.cExtras ? `${backendCompanions.cExtras}` : '';

    const parsedMods = promptConfig?.backendMods;
    ...
    const xRpg = parsedMods?.xRpg === 'true' ?
    `
      You will now play an adventure game with the user.
      In your response, you can give the user missions with choices that affect the adventure.

      Every choice should:
      - give the user money or made-up items depending on the choice outcome
      - either add or subtract a heart/health point depending on the choice outcome
      - give the user xp points

      Only use the functions given when an event happens in the adventure and refuse to use the functions if the user asks for it directly.

      Use the function calling methods you were given in the following format for JSON: <!>FUNCTION GOES INSIDE HERE</!>.

      Here are 2 examples:

      <!>{"functions.addToWallet": {"amount": 10}}</!>

      <!>{"functions.addToWallet": {"amount": 100},"functions.modifyHealth": {"value": -1}}</!>

      Only do the functions IN BETWEEN the tags when calling functions AND NEVER USE MARKDOWN FOR IT.
      ` : '';
  ...
  const sanitizedQuery = query.trim()
   ...
   if (isFlagged) {
    ...
    const useFunctions = xRpg ? { functions } : {};
